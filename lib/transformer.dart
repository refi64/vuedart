...
class VueTransformer extends Transformer {
  final BarbackSettings _settings;

  VueTransformer.asPlugin(this._settings) {}

  String get allowedExtensions => '.dart .html';
  ...
  Future apply(Transform transform) async {
    if (transform.primaryInput.id.extension == '.dart') {
      return _applyDart(transform);
    } else {
      return _applyHtml(transform);
    }
  }

  Future _applyDart(Transform transform) async {
...
${components.map((comp) =>
  ...
  Future _applyHtml(Transform transform) async {
    var doc = parse(await transform.primaryInput.readAsString());
    var children = doc.body.children;
    if (!(!children.isEmpty && children[0].localName == 'template' &&
          children[0].attributes.containsKey('vuedart'))) {
      transform.addOutput(transform.primaryInput);
    } else print('Skipped ${transform.primaryInput.id.path}!');
